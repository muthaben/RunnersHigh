...
function Map ({ getLongitude, getLatitude }) {
  const address = [Number(getLatitude), Number(getLongitude)]
  useEffect(() => {
    const mapContainer = document.getElementById('map') // 지도를 표시할 div
    const mapOption = {
      center: new kakao.maps.LatLng(address[0], address[1]), // 지도의 중심좌표
      level: 3 // 지도의 확대 레벨
    }
    ...
    kakao.maps.event.addListener(map, 'mousemove', function (mouseEvent) {
      ...
      if (drawingFlag) {
        ...
        const content = '<div class="dotOverlay distanceInfo">총거리 <span class="number">' + distance + '</span>m</div>' // 커스텀오버레이에 추가될 내용입니다
    ...
    kakao.maps.event.addListener(map, 'rightclick', function (mouseEvent) {
      ...
      if (drawingFlag) {
        ...
        if (path.length > 1) {
          ...
          const content = getTimeHTML(distance) // 커스텀오버레이에 추가될 내용입니다
    ...
    function getTimeHTML (distance) {
      ...
      let bycicleHour = ''; let bycicleMin = ''
